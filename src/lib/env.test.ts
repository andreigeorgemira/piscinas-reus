...
describe('getSiteUrl', () => {
  ...
  it('returns the site origin', async () => {
    vi.stubEnv('SITE_URL', 'https://piscinas-reus.vercel.app')
    const { getSiteUrl } = await import('./env')
    expect(getSiteUrl()).toBe('https://piscinas-reus.vercel.app')
  })

  it('rejects a site url that is not a url', async () => {
    vi.stubEnv('SITE_URL', 'piscinas-reus.vercel.app')
    const { getSiteUrl } = await import('./env')
    expect(() => getSiteUrl()).toThrowError(/SITE_URL/)
  })
})
